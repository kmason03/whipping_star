...
const int dm2_grdpts(400), ue4_grdpts(25), umu4_grdpts(25);
...
//	auto time_a = std::chrono::steady_clock::now();
	...
	if(!gen){
		...
		coordfile.open("bins_full_220207.txt", std::ios_base::app);
		chifile.open("chis_full_220125.txt", std::ios_base::app);
		covfile.open("cov_full_220207.txt", std::ios_base::app);
		...
		TFile * fsys = new TFile("data/systematics/DL_full.SBNcovar.root","read");
