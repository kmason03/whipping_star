...
cp ${WORK_DIR}/DL3plus1_fakedata .

if [ ${arrayid} -eq 0 ]; then
# run your command
./DL3plus1_fakedata 1 0 >${WORK_DIR}/logs/log_${arrayid}.txt
mv *sinee*txt  ${WORK_DIR}/.
fi

if [ ${arrayid} -eq 1 ]; then
# run your command
./DL3plus1_fakedata 1 1 >${WORK_DIR}/logs/log_${arrayid}.txt
mv *sinmu*txt  ${WORK_DIR}/.
fi

if [ ${arrayid} -eq 2 ]; then 
# run your command
./DL3plus1_fakedata 1 2 >${WORK_DIR}/logs/log_${arrayid}.txt
mv *sinemu*txt  ${WORK_DIR}/.
fi

if [ ${arrayid} -eq 3 ]; then
# run your command
./DL3plus1_fakedata 2 0 >${WORK_DIR}/logs/log_${arrayid}.txt
mv *sinee*txt  ${WORK_DIR}/.
fi

if [ ${arrayid} -eq 4 ]; then
# run your command
./DL3plus1_fakedata 2 1 >${WORK_DIR}/logs/log_${arrayid}.txt
mv *sinmu*txt  ${WORK_DIR}/.
fi

if [ ${arrayid} -eq 5 ]; then
# run your command
./DL3plus1_fakedata 2 2 >${WORK_DIR}/logs/log_${arrayid}.txt
mv *sinemu*txt  ${WORK_DIR}/.
fi
