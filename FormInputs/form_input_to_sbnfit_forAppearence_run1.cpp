...
int main() {
  ...
  // ... for event lists
  const char* sel_bnb_eventlist_file      = "/uboone/app/users/kmason/LaurenVersion/form_input_to_sbnfit/sel1e1p_FinalSelection_bnb_run1_events.txt";
  const char* sel_bnb_lowE_eventlist_file = "/uboone/app/users/kmason/LaurenVersion/form_input_to_sbnfit/empty.txt";
  const char* sel_nue_eventlist_file      = "/uboone/app/users/kmason/LaurenVersion/form_input_to_sbnfit/sel1e1p_FinalSelection_nue_run1_events.txt";
  const char* sel_nue_lowE_eventlist_file = "/uboone/app/users/kmason/LaurenVersion/form_input_to_sbnfit/empty.txt";
  const char* sel_ncpi0_eventlist_file    = "/uboone/app/users/kmason/LaurenVersion/form_input_to_sbnfit/sel1e1p_FinalSelection_ncpi0_run1_events.txt";
  const char* sel_ccpi0_eventlist_file    = "/uboone/app/users/kmason/LaurenVersion/form_input_to_sbnfit/sel1e1p_FinalSelection_ccpi0_run1_events.txt";
  const char* sel_dirt_eventlist_file     = "/uboone/app/users/kmason/LaurenVersion/form_input_to_sbnfit/empty.txt";
  const char* sel_extbnb_eventlist_file   = "/uboone/app/users/kmason/LaurenVersion/form_input_to_sbnfit/sel1e1p_FinalSelection_extbnb_run1_events.txt";
  const char* sel_fullosc_eventlist_file   = "/uboone/app/users/kmason/LaurenVersion/form_input_to_sbnfit/sel1e1p_FinalSelection_fullosc_run1_events.txt";
  // ... for data event list
  ...
  // ... for arborist files
  const char* bnb_arborist_file  = "/uboone/data/users/yatesla/othersys_mcc9/arborist/arborist_v48_Sep24_withExtraGENIE_bnb_nu_run1.root";
  const char* bnb_lowE_arborist_file  = "/uboone/data/users/yatesla/othersys_mcc9/arborist/arborist_v48_Sep24_withExtraGENIE_bnb_nu_lowE_run1.root";
  const char* nue_arborist_file       = "/uboone/data/users/yatesla/othersys_mcc9/arborist/arborist_v48_Sep24_withExtraGENIE_intrinsic_nue_run1.root";
  const char* nue_lowE_arborist_file  = "/uboone/data/users/yatesla/othersys_mcc9/arborist/arborist_v48_Sep24_withExtraGENIE_intrinsic_nue_lowE_run1.root";
  const char* ncpi0_arborist_file     = "/uboone/data/users/yatesla/othersys_mcc9/arborist/arborist_v48_Sep24_withExtraGENIE_nc_pi0_run1.root";
  const char* ccpi0_arborist_file     = "/uboone/data/users/yatesla/othersys_mcc9/arborist/arborist_v48_Sep24_withExtraGENIE_cc_pi0_run1.root";
  const char* fullosc_arborist_file     = "/uboone/data/users/kmason/arborist/arborist_v48_Sep24_withExtraGENIE_fullosc_nue_run1.root";
  const char* dirt_arborist_file      = "/uboone/data/users/yatesla/othersys_mcc9/arborist/arborist_v48_Sep24_withExtraGENIE_dirt_nu_run1.root";
  // ... for output
  const char* output_file = "/uboone/app/users/kmason/LaurenVersion/form_input_to_sbnfit/input_to_sbnfit_v48_Oct27_withExtraGENIE_1e1p_FinalSelection_run1.root";
  ...
  std::map<std::array<int,3>,std::array<double,N_var>> sel_fullosc_event_map;
  ...
  // Read in the fullosc event list
  std::ifstream sel_fullosc_eventlist(sel_fullosc_eventlist_file);
  if ( sel_fullosc_eventlist.is_open() ) {
     while ( getline(sel_fullosc_eventlist, line) ) {
        boost::algorithm::split(split_line, line, boost::is_any_of(","));
        rse[0] = std::stoi(split_line[0]);
        rse[1] = std::stoi(split_line[1]);
        rse[2] = std::stoi(split_line[2]);
        for ( int i=0; i<N_var; i++ ) reco_var[i] = std::stof(split_line[i+4]);  // skip vtxid after RSE
        sel_fullosc_event_map.emplace(rse,reco_var);
     }
    sel_fullosc_eventlist.close();
  }
  std::cout << "Read in " << sel_fullosc_event_map.size() << " selected events from the fullosc overlay sample" << std::endl;  
  ...
  // ... for fullosc
  TTree* sel_fullosc_output_tree = new TTree("sel_fullosc_tree", "events selected from fullosc overlay sample");
  sel_fullosc_output_tree->Branch("run",&run);
  sel_fullosc_output_tree->Branch("subrun",&subrun);
  sel_fullosc_output_tree->Branch("event",&event);
  sel_fullosc_output_tree->Branch("nu_pdg",&nu_pdg);
  sel_fullosc_output_tree->Branch("nu_energy_true",&nu_energy_true);
  sel_fullosc_output_tree->Branch("nu_L_true",&nu_L_true);
  sel_fullosc_output_tree->Branch("nu_interaction_ccnc",&nu_interaction_ccnc);
  sel_fullosc_output_tree->Branch("nu_interaction_mode",&nu_interaction_mode);
  sel_fullosc_output_tree->Branch("nu_interaction_type",&nu_interaction_type);
  sel_fullosc_output_tree->Branch("nu_target_pdg",&nu_target_pdg);
  sel_fullosc_output_tree->Branch("tune1_weight",&tune1_weight);
  sel_fullosc_output_tree->Branch("spline_weight",&spline_weight);
  sel_fullosc_output_tree->Branch("rootino_weight",&rootino_weight);
  sel_fullosc_output_tree->Branch("ub_tune_weight",&ub_tune_weight);
  sel_fullosc_output_tree->Branch("xsec_corr_weight",&xsec_corr_weight);
  sel_fullosc_output_tree->Branch("lee_weight",&lee_weight);
  sel_fullosc_output_tree->Branch("sys_weights",&sys_weights);
  sel_fullosc_output_tree->Branch("nu_energy_reco",&reco_var[0]);
  sel_fullosc_output_tree->Branch("eta_reco",&reco_var[1]);
  sel_fullosc_output_tree->Branch("pT_reco",&reco_var[2]);
  sel_fullosc_output_tree->Branch("alphaT_reco",&reco_var[3]);
  sel_fullosc_output_tree->Branch("sphB_reco",&reco_var[4]);
  sel_fullosc_output_tree->Branch("pzEnu_reco",&reco_var[5]);
  sel_fullosc_output_tree->Branch("charge_near_trunk_reco",&reco_var[6]);
  sel_fullosc_output_tree->Branch("Q0_reco",&reco_var[7]);
  sel_fullosc_output_tree->Branch("Q3_reco",&reco_var[8]);
  sel_fullosc_output_tree->Branch("sum_thetas_reco",&reco_var[9]);
  sel_fullosc_output_tree->Branch("sum_phis_reco",&reco_var[10]);
  sel_fullosc_output_tree->Branch("pT_ratio_reco",&reco_var[11]);
  sel_fullosc_output_tree->Branch("proton_theta_reco",&reco_var[12]);
  sel_fullosc_output_tree->Branch("proton_phi_reco",&reco_var[13]);
  sel_fullosc_output_tree->Branch("min_shr_frac_reco",&reco_var[14]);
  sel_fullosc_output_tree->Branch("max_shr_frac_reco",&reco_var[15]);
  sel_fullosc_output_tree->Branch("BjxB_reco",&reco_var[16]);
  sel_fullosc_output_tree->Branch("BjyB_reco",&reco_var[17]);
  sel_fullosc_output_tree->Branch("proton_KE_reco",&reco_var[18]);
  sel_fullosc_output_tree->Branch("lepton_KE_reco",&reco_var[19]);
  sel_fullosc_output_tree->Branch("lepton_theta_reco",&reco_var[20]);
  sel_fullosc_output_tree->Branch("lepton_phi_reco",&reco_var[21]);
  sel_fullosc_output_tree->Branch("openang_reco",&reco_var[22]);
  sel_fullosc_output_tree->Branch("x_reco",&reco_var[23]);
  sel_fullosc_output_tree->Branch("y_reco",&reco_var[24]);
  sel_fullosc_output_tree->Branch("z_reco",&reco_var[25]);
  sel_fullosc_output_tree->Branch("mpid_muon_score",&reco_var[26]);
  sel_fullosc_output_tree->Branch("mpid_proton_score",&reco_var[27]);
  sel_fullosc_output_tree->Branch("mpid_electron_score",&reco_var[28]);
  sel_fullosc_output_tree->Branch("shr_charge_ratio_reco",&reco_var[29]);
  sel_fullosc_output_tree->Branch("shr_consistency_reco",&reco_var[30]);
  sel_fullosc_output_tree->Branch("nu_energy_QE_lepton_reco",&reco_var[31]);
  sel_fullosc_output_tree->Branch("nu_energy_QE_proton_reco",&reco_var[32]);
  sel_fullosc_output_tree->Branch("proton_length_reco",&reco_var[33]);
  sel_fullosc_output_tree->Branch("lepton_length_reco",&reco_var[34]);
  // skip reco_var[35], which is Nick's copy of the true neutrino energy
  // skip reco_var[36], which is Nick's copy of the ccnc flag
  // skip reco_var[37], which is Nick's copy of the interaction type
  sel_fullosc_output_tree->Branch("dllee_scedr",&reco_var[38]);
  sel_fullosc_output_tree->Branch("dllee_pot_weight",&reco_var[39]);
  // skip reco_var[40], which is Nick's copy of the CV tune weight
  // skip reco_var[41], which is Nick's copy of the LEE weight
  sel_fullosc_output_tree->Branch("dllee_pi0_flag_new",&reco_var[42]);
  sel_fullosc_output_tree->Branch("dllee_pi0_flag_old",&reco_var[43]);
  // skip reco_var[44], which is the Run number
  sel_fullosc_output_tree->Branch("dllee_cut_level",&reco_var[45]);
  sel_fullosc_output_tree->Branch("dllee_nBDTs",&reco_var[46]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight00",&reco_var[47]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score00",&reco_var[48]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight01",&reco_var[49]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score01",&reco_var[50]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight02",&reco_var[51]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score02",&reco_var[52]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight03",&reco_var[53]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score03",&reco_var[54]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight04",&reco_var[55]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score04",&reco_var[56]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight05",&reco_var[57]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score05",&reco_var[58]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight06",&reco_var[59]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score06",&reco_var[60]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight07",&reco_var[61]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score07",&reco_var[62]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight08",&reco_var[63]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score08",&reco_var[64]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight09",&reco_var[65]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score09",&reco_var[66]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight10",&reco_var[67]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score10",&reco_var[68]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight11",&reco_var[69]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score11",&reco_var[70]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight12",&reco_var[71]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score12",&reco_var[72]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight13",&reco_var[73]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score13",&reco_var[74]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight14",&reco_var[75]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score14",&reco_var[76]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight15",&reco_var[77]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score15",&reco_var[78]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight16",&reco_var[79]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score16",&reco_var[80]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight17",&reco_var[81]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score17",&reco_var[82]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight18",&reco_var[83]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score18",&reco_var[84]);
  sel_fullosc_output_tree->Branch("dllee_bdt_tvweight19",&reco_var[85]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score19",&reco_var[86]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score_avg",&reco_var[87]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score_median",&reco_var[88]);
  sel_fullosc_output_tree->Branch("dllee_bdt_score_max",&reco_var[89]);
  // skip reco_var[90] and reco_var[91], which are not clear to me
  sel_fullosc_output_tree->Branch("dllee_pi0_weight",&reco_var[92]);
 ...
 // Read in bnb nu arborist file
 TFile* fullosc_arborist = new TFile(fullosc_arborist_file, "READ");
 if ( !fullosc_arborist->IsOpen() ) std::cout << "Failed to open file " << fullosc_arborist_file << std::endl;
 
 // Get the desired tree
 TTree* fullosc_evweight_tree = (TTree*)fullosc_arborist->Get("arborist/eventweight_tree");
 fullosc_evweight_tree->SetBranchAddress("run",&run);
 fullosc_evweight_tree->SetBranchAddress("subrun",&subrun);
 fullosc_evweight_tree->SetBranchAddress("event",&event);
 fullosc_evweight_tree->SetBranchAddress("nu_pdg",&nu_pdg);
 fullosc_evweight_tree->SetBranchAddress("nu_energy_true",&nu_energy_true);
 fullosc_evweight_tree->SetBranchAddress("nu_interaction_ccnc",&nu_interaction_ccnc);
 fullosc_evweight_tree->SetBranchAddress("nu_interaction_mode",&nu_interaction_mode);
 fullosc_evweight_tree->SetBranchAddress("nu_interaction_type",&nu_interaction_type);
 fullosc_evweight_tree->SetBranchAddress("nu_target_pdg",&nu_target_pdg);
 fullosc_evweight_tree->SetBranchAddress("nu_L_true",&nu_L_true);
 fullosc_evweight_tree->SetBranchAddress("spline_weight",&spline_weight);
 fullosc_evweight_tree->SetBranchAddress("rootino_weight",&rootino_weight);
 fullosc_evweight_tree->SetBranchAddress("ub_tune_weight",&ub_tune_weight);
 fullosc_evweight_tree->SetBranchAddress("xsec_corr_weight",&xsec_corr_weight);
 fullosc_evweight_tree->SetBranchAddress("lee_weight",&lee_weight);
 fullosc_evweight_tree->SetBranchAddress("sys_weights",&sys_weights_ptr);
 
 // Loop over the bnb nu tree
 for ( int i=0; i < fullosc_evweight_tree->GetEntries(); i++ ) {
    std::cout<<"entry: "<<i<<std::endl;
    // Get the entry
    fullosc_evweight_tree->GetEntry(i);
    ...
    // If there's a corresponding entry in the selected map...
    if ( sel_fullosc_event_map.find(rse) != sel_fullosc_event_map.end() ) {
       // Get the reconstructed variables
       reco_var = sel_fullosc_event_map.find(rse)->second;
       // Check that the true energies agree... If the true energy is off by more than 0.01 MeV, this is not the right entry in the tree and we skip it
       if ( std::abs(nu_energy_true - reco_var[35]) > 0.01 ) continue;
       // Get the Tune 1 weight
       if ( nu_interaction_type == 1001 ) {
       ...
       if ( std::isinf(ub_tune_weight) || ub_tune_weight < 0. ) {
           if ( std::isinf(ub_tune_weight) )
           std::cout << "Event (" << run << ", " << subrun << ", " << event << ") in sel_fullosc has infinite ub_tune_weight... Setting to 1" << std::endl;
           if ( ub_tune_weight < 0. )
           std::cout << "Event (" << run << ", " << subrun << ", " << event << ") in sel_fullosc has negative ub_tune_weight... Setting to 1" << std::endl;
           ub_tune_weight = 1.;
           xsec_corr_weight = spline_weight * rootino_weight;
           for (const auto& sys_weight_pair : sys_weights) { for (auto weight : sys_weight_pair.second) { weight = 1.; } }
       }
       // Fill the tree
       sel_fullosc_output_tree->Fill();
    }
 }             
std::cout << "Filled fullosc tree" << std::endl;
...
/*
  ...
  */
  ...
  int n_sel_fullosc_in       = sel_fullosc_event_map.size();
  int n_sel_fullosc_out      = sel_fullosc_output_tree->GetEntries();
  ...
  std::cout << "  Fullosc overlay events: " << n_sel_fullosc_in << " events in, " << n_sel_fullosc_out << " events out" << std::endl; 
