...
int main() {
  ...
  // ... for event lists
  const char* sel_bnb_eventlist_file      = "/uboone/app/users/yatesla/othersys_mcc9/input_to_sbnfit/sel1mu1p_bnb_run2_eventlist.txt";
  const char* sel_bnb_lowE_eventlist_file = "/uboone/app/users/yatesla/othersys_mcc9/input_to_sbnfit/empty.txt";
  const char* sel_nue_eventlist_file      = "/uboone/app/users/yatesla/othersys_mcc9/input_to_sbnfit/sel1mu1p_nue_run2_eventlist.txt";
  const char* sel_nue_lowE_eventlist_file = "/uboone/app/users/yatesla/othersys_mcc9/input_to_sbnfit/empty.txt";
  const char* sel_ncpi0_eventlist_file    = "/uboone/app/users/yatesla/othersys_mcc9/input_to_sbnfit/sel1mu1p_ncpi0_run2_eventlist.txt";
  const char* sel_ccpi0_eventlist_file    = "/uboone/app/users/yatesla/othersys_mcc9/input_to_sbnfit/sel1mu1p_ccpi0_run2_eventlist.txt";
  const char* sel_dirt_eventlist_file     = "/uboone/app/users/yatesla/othersys_mcc9/input_to_sbnfit/empty.txt";
  const char* sel_extbnb_eventlist_file   = "/uboone/app/users/yatesla/othersys_mcc9/input_to_sbnfit/sel1mu1p_extbnb_run2_eventlist.txt";
  // ... for data event list
  const char* sel_data_eventlist_file = "/uboone/app/users/yatesla/othersys_mcc9/input_to_sbnfit/empty.txt";
  // ... for Tune 1 weights
  ...
  // ... for arborist files
  const char* bnb_arborist_file       = "/uboone/data/users/yatesla/othersys_mcc9/arborist/arborist_v55_Jun20_withExtraGENIE_bnb_nu_run2.root";
  const char* bnb_lowE_arborist_file  = "/uboone/data/users/yatesla/othersys_mcc9/arborist/arborist_v55_Jun20_withExtraGENIE_bnb_nu_lowE_run2.root";
  const char* nue_arborist_file       = "/uboone/data/users/yatesla/othersys_mcc9/arborist/arborist_v55_Jun20_withExtraGENIE_intrinsic_nue_run2.root";
  const char* nue_lowE_arborist_file  = "/uboone/data/users/yatesla/othersys_mcc9/arborist/arborist_v55_Jun20_withExtraGENIE_intrinsic_nue_lowE_run2.root";
  const char* ncpi0_arborist_file     = "/uboone/data/users/yatesla/othersys_mcc9/arborist/arborist_v55_Jun20_withExtraGENIE_nc_pi0_run2.root";
  const char* ccpi0_arborist_file     = "/uboone/data/users/yatesla/othersys_mcc9/arborist/arborist_v55_Jun20_withExtraGENIE_cc_pi0_run2.root";
  const char* dirt_arborist_file      = "/uboone/data/users/yatesla/othersys_mcc9/arborist/arborist_v55_Jun20_withExtraGENIE_dirt_nu_run2.root";
  // ... for output
  const char* output_file = "/uboone/app/users/jmills/form_input_to_sbnfit/src/outputs_for_numudis/input_to_sbnfit_v55_Nov15_withExtraGENIE_1mu1p_run2_Aug06.root";

  // Initalize the number of reco variables
  const int N_var = 79;  // 42 reco(-ish) variables + true energy + ccnc flag + scedr + "POT weight" + CV weight + LEE weight + pi0 flag + Run number + BDT score information (26 variables) + 2 more reco variables + pi0 weight
  //const int N_var = 2;
  ...
  sel_bnb_output_tree->Branch("phiT_reco",&reco_var[29]);
  sel_bnb_output_tree->Branch("Q2_reco",&reco_var[30]);
  sel_bnb_output_tree->Branch("lepton_length_reco",&reco_var[31]);
  sel_bnb_output_tree->Branch("proton_length_reco",&reco_var[32]);
  sel_bnb_output_tree->Branch("proton_cos_theta_reco",&reco_var[33]);
  sel_bnb_output_tree->Branch("lepton_cos_theta_reco",&reco_var[34]);
  sel_bnb_output_tree->Branch("x_true",&reco_var[35]);
  sel_bnb_output_tree->Branch("y_true",&reco_var[36]);
  sel_bnb_output_tree->Branch("z_true",&reco_var[37]);
  sel_bnb_output_tree->Branch("dllee_pass_simple_cuts_reco",&reco_var[38]);
  sel_bnb_output_tree->Branch("dllee_failed_boost_reco",&reco_var[39]);
  sel_bnb_output_tree->Branch("tot_PE_reco",&reco_var[40]);
  sel_bnb_output_tree->Branch("porch_tot_PE_reco",&reco_var[41]);
  // skip reco_var[42], which is Josh's copy of the true neutrino energy
  // skip reco_var[43], which is Josh's copy of the ccnc flag
  sel_bnb_output_tree->Branch("dllee_scedr",&reco_var[44]);
  sel_bnb_output_tree->Branch("dllee_pot_weight",&reco_var[45]);
  // skip reco_var[46], which is Josh's copy of the CV tune weight
  // skip reco_var[47], which is Josh's copy of the LEE weight
  sel_bnb_output_tree->Branch("dllee_pi0_flag",&reco_var[48]);
  // skip reco_var[49], which is the Run number
  sel_bnb_output_tree->Branch("dllee_nBDTs",&reco_var[50]);
  sel_bnb_output_tree->Branch("dllee_bdt_tvweight00",&reco_var[51]);
  sel_bnb_output_tree->Branch("dllee_bdt_score00",&reco_var[52]);
  sel_bnb_output_tree->Branch("dllee_bdt_tvweight01",&reco_var[53]);
  sel_bnb_output_tree->Branch("dllee_bdt_score01",&reco_var[54]);
  sel_bnb_output_tree->Branch("dllee_bdt_tvweight02",&reco_var[55]);
  sel_bnb_output_tree->Branch("dllee_bdt_score02",&reco_var[56]);
  sel_bnb_output_tree->Branch("dllee_bdt_tvweight03",&reco_var[57]);
  sel_bnb_output_tree->Branch("dllee_bdt_score03",&reco_var[58]);
  sel_bnb_output_tree->Branch("dllee_bdt_tvweight04",&reco_var[59]);
  sel_bnb_output_tree->Branch("dllee_bdt_score04",&reco_var[60]);
  sel_bnb_output_tree->Branch("dllee_bdt_tvweight05",&reco_var[61]);
  sel_bnb_output_tree->Branch("dllee_bdt_score05",&reco_var[62]);
  sel_bnb_output_tree->Branch("dllee_bdt_tvweight06",&reco_var[63]);
  sel_bnb_output_tree->Branch("dllee_bdt_score06",&reco_var[64]);
  sel_bnb_output_tree->Branch("dllee_bdt_tvweight07",&reco_var[65]);
  sel_bnb_output_tree->Branch("dllee_bdt_score07",&reco_var[66]);
  sel_bnb_output_tree->Branch("dllee_bdt_tvweight08",&reco_var[67]);
  sel_bnb_output_tree->Branch("dllee_bdt_score08",&reco_var[68]);
  sel_bnb_output_tree->Branch("dllee_bdt_tvweight09",&reco_var[69]);
  sel_bnb_output_tree->Branch("dllee_bdt_score09",&reco_var[70]);
  sel_bnb_output_tree->Branch("dllee_bdt_score_avg",&reco_var[71]);
  sel_bnb_output_tree->Branch("dllee_bdt_score_median",&reco_var[72]);
  sel_bnb_output_tree->Branch("dllee_bdt_score_max",&reco_var[73]);
  // skip reco_var[74] and reco_var[75], which are not clear to me
  sel_bnb_output_tree->Branch("nu_energy_QE_lepton_reco",&reco_var[76]);
  sel_bnb_output_tree->Branch("nu_energy_QE_proton_reco",&reco_var[77]);
  sel_bnb_output_tree->Branch("dllee_pi0_weight",&reco_var[78]);
  // ... for bnb nu lowE
  ...
  sel_bnb_lowE_output_tree->Branch("phiT_reco",&reco_var[29]);
  sel_bnb_lowE_output_tree->Branch("Q2_reco",&reco_var[30]);
  sel_bnb_lowE_output_tree->Branch("lepton_length_reco",&reco_var[31]);
  sel_bnb_lowE_output_tree->Branch("proton_length_reco",&reco_var[32]);
  sel_bnb_lowE_output_tree->Branch("proton_cos_theta_reco",&reco_var[33]);
  sel_bnb_lowE_output_tree->Branch("lepton_cos_theta_reco",&reco_var[34]);
  sel_bnb_lowE_output_tree->Branch("x_true",&reco_var[35]);
  sel_bnb_lowE_output_tree->Branch("y_true",&reco_var[36]);
  sel_bnb_lowE_output_tree->Branch("z_true",&reco_var[37]);
  sel_bnb_lowE_output_tree->Branch("dllee_pass_simple_cuts_reco",&reco_var[38]);
  sel_bnb_lowE_output_tree->Branch("dllee_failed_boost_reco",&reco_var[39]);
  sel_bnb_lowE_output_tree->Branch("tot_PE_reco",&reco_var[40]);
  sel_bnb_lowE_output_tree->Branch("porch_tot_PE_reco",&reco_var[41]);
  sel_bnb_lowE_output_tree->Branch("dllee_scedr",&reco_var[44]);
  sel_bnb_lowE_output_tree->Branch("dllee_pot_weight",&reco_var[45]);
  sel_bnb_lowE_output_tree->Branch("dllee_pi0_flag",&reco_var[48]);
  sel_bnb_lowE_output_tree->Branch("dllee_nBDTs",&reco_var[50]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_tvweight00",&reco_var[51]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_score00",&reco_var[52]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_tvweight01",&reco_var[53]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_score01",&reco_var[54]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_tvweight02",&reco_var[55]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_score02",&reco_var[56]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_tvweight03",&reco_var[57]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_score03",&reco_var[58]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_tvweight04",&reco_var[59]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_score04",&reco_var[60]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_tvweight05",&reco_var[61]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_score05",&reco_var[62]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_tvweight06",&reco_var[63]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_score06",&reco_var[64]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_tvweight07",&reco_var[65]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_score07",&reco_var[66]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_tvweight08",&reco_var[67]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_score08",&reco_var[68]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_tvweight09",&reco_var[69]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_score09",&reco_var[70]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_score_avg",&reco_var[71]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_score_median",&reco_var[72]);
  sel_bnb_lowE_output_tree->Branch("dllee_bdt_score_max",&reco_var[73]);
  sel_bnb_lowE_output_tree->Branch("nu_energy_QE_lepton_reco",&reco_var[76]);
  sel_bnb_lowE_output_tree->Branch("nu_energy_QE_proton_reco",&reco_var[77]);
  sel_bnb_lowE_output_tree->Branch("dllee_pi0_weight",&reco_var[78]);
  // ... for intrinsic nue
  ...
  sel_nue_output_tree->Branch("phiT_reco",&reco_var[29]);
  sel_nue_output_tree->Branch("Q2_reco",&reco_var[30]);
  sel_nue_output_tree->Branch("lepton_length_reco",&reco_var[31]);
  sel_nue_output_tree->Branch("proton_length_reco",&reco_var[32]);
  sel_nue_output_tree->Branch("proton_cos_theta_reco",&reco_var[33]);
  sel_nue_output_tree->Branch("lepton_cos_theta_reco",&reco_var[34]);
  sel_nue_output_tree->Branch("x_true",&reco_var[35]);
  sel_nue_output_tree->Branch("y_true",&reco_var[36]);
  sel_nue_output_tree->Branch("z_true",&reco_var[37]);
  sel_nue_output_tree->Branch("dllee_pass_simple_cuts_reco",&reco_var[38]);
  sel_nue_output_tree->Branch("dllee_failed_boost_reco",&reco_var[39]);
  sel_nue_output_tree->Branch("tot_PE_reco",&reco_var[40]);
  sel_nue_output_tree->Branch("porch_tot_PE_reco",&reco_var[41]);
  sel_nue_output_tree->Branch("dllee_scedr",&reco_var[44]);
  sel_nue_output_tree->Branch("dllee_pot_weight",&reco_var[45]);
  sel_nue_output_tree->Branch("dllee_pi0_flag",&reco_var[48]);
  sel_nue_output_tree->Branch("dllee_nBDTs",&reco_var[50]);
  sel_nue_output_tree->Branch("dllee_bdt_tvweight00",&reco_var[51]);
  sel_nue_output_tree->Branch("dllee_bdt_score00",&reco_var[52]);
  sel_nue_output_tree->Branch("dllee_bdt_tvweight01",&reco_var[53]);
  sel_nue_output_tree->Branch("dllee_bdt_score01",&reco_var[54]);
  sel_nue_output_tree->Branch("dllee_bdt_tvweight02",&reco_var[55]);
  sel_nue_output_tree->Branch("dllee_bdt_score02",&reco_var[56]);
  sel_nue_output_tree->Branch("dllee_bdt_tvweight03",&reco_var[57]);
  sel_nue_output_tree->Branch("dllee_bdt_score03",&reco_var[58]);
  sel_nue_output_tree->Branch("dllee_bdt_tvweight04",&reco_var[59]);
  sel_nue_output_tree->Branch("dllee_bdt_score04",&reco_var[60]);
  sel_nue_output_tree->Branch("dllee_bdt_tvweight05",&reco_var[61]);
  sel_nue_output_tree->Branch("dllee_bdt_score05",&reco_var[62]);
  sel_nue_output_tree->Branch("dllee_bdt_tvweight06",&reco_var[63]);
  sel_nue_output_tree->Branch("dllee_bdt_score06",&reco_var[64]);
  sel_nue_output_tree->Branch("dllee_bdt_tvweight07",&reco_var[65]);
  sel_nue_output_tree->Branch("dllee_bdt_score07",&reco_var[66]);
  sel_nue_output_tree->Branch("dllee_bdt_tvweight08",&reco_var[67]);
  sel_nue_output_tree->Branch("dllee_bdt_score08",&reco_var[68]);
  sel_nue_output_tree->Branch("dllee_bdt_tvweight09",&reco_var[69]);
  sel_nue_output_tree->Branch("dllee_bdt_score09",&reco_var[70]);
  sel_nue_output_tree->Branch("dllee_bdt_score_avg",&reco_var[71]);
  sel_nue_output_tree->Branch("dllee_bdt_score_median",&reco_var[72]);
  sel_nue_output_tree->Branch("dllee_bdt_score_max",&reco_var[73]);
  sel_nue_output_tree->Branch("nu_energy_QE_lepton_reco",&reco_var[76]);
  sel_nue_output_tree->Branch("nu_energy_QE_proton_reco",&reco_var[77]);
  sel_nue_output_tree->Branch("dllee_pi0_weight",&reco_var[78]);
  ...
  sel_nue_lowE_output_tree->Branch("phiT_reco",&reco_var[29]);
  sel_nue_lowE_output_tree->Branch("Q2_reco",&reco_var[30]);
  sel_nue_lowE_output_tree->Branch("lepton_length_reco",&reco_var[31]);
  sel_nue_lowE_output_tree->Branch("proton_length_reco",&reco_var[32]);
  sel_nue_lowE_output_tree->Branch("proton_cos_theta_reco",&reco_var[33]);
  sel_nue_lowE_output_tree->Branch("lepton_cos_theta_reco",&reco_var[34]);
  sel_nue_lowE_output_tree->Branch("x_true",&reco_var[35]);
  sel_nue_lowE_output_tree->Branch("y_true",&reco_var[36]);
  sel_nue_lowE_output_tree->Branch("z_true",&reco_var[37]);
  sel_nue_lowE_output_tree->Branch("dllee_pass_simple_cuts_reco",&reco_var[38]);
  sel_nue_lowE_output_tree->Branch("dllee_failed_boost_reco",&reco_var[39]);
  sel_nue_lowE_output_tree->Branch("tot_PE_reco",&reco_var[40]);
  sel_nue_lowE_output_tree->Branch("porch_tot_PE_reco",&reco_var[41]);
  sel_nue_lowE_output_tree->Branch("dllee_scedr",&reco_var[44]);
  sel_nue_lowE_output_tree->Branch("dllee_pot_weight",&reco_var[45]);
  sel_nue_lowE_output_tree->Branch("dllee_pi0_flag",&reco_var[48]);
  sel_nue_lowE_output_tree->Branch("dllee_nBDTs",&reco_var[50]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_tvweight00",&reco_var[51]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_score00",&reco_var[52]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_tvweight01",&reco_var[53]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_score01",&reco_var[54]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_tvweight02",&reco_var[55]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_score02",&reco_var[56]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_tvweight03",&reco_var[57]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_score03",&reco_var[58]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_tvweight04",&reco_var[59]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_score04",&reco_var[60]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_tvweight05",&reco_var[61]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_score05",&reco_var[62]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_tvweight06",&reco_var[63]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_score06",&reco_var[64]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_tvweight07",&reco_var[65]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_score07",&reco_var[66]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_tvweight08",&reco_var[67]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_score08",&reco_var[68]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_tvweight09",&reco_var[69]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_score09",&reco_var[70]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_score_avg",&reco_var[71]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_score_median",&reco_var[72]);
  sel_nue_lowE_output_tree->Branch("dllee_bdt_score_max",&reco_var[73]);
  sel_nue_lowE_output_tree->Branch("nu_energy_QE_lepton_reco",&reco_var[76]);
  sel_nue_lowE_output_tree->Branch("nu_energy_QE_proton_reco",&reco_var[77]);
  sel_nue_lowE_output_tree->Branch("dllee_pi0_weight",&reco_var[78]);
  // ... for NC pi0
  ...
  sel_ncpi0_output_tree->Branch("phiT_reco",&reco_var[29]);
  sel_ncpi0_output_tree->Branch("Q2_reco",&reco_var[30]);
  sel_ncpi0_output_tree->Branch("lepton_length_reco",&reco_var[31]);
  sel_ncpi0_output_tree->Branch("proton_length_reco",&reco_var[32]);
  sel_ncpi0_output_tree->Branch("proton_cos_theta_reco",&reco_var[33]);
  sel_ncpi0_output_tree->Branch("lepton_cos_theta_reco",&reco_var[34]);
  sel_ncpi0_output_tree->Branch("x_true",&reco_var[35]);
  sel_ncpi0_output_tree->Branch("y_true",&reco_var[36]);
  sel_ncpi0_output_tree->Branch("z_true",&reco_var[37]);
  sel_ncpi0_output_tree->Branch("dllee_pass_simple_cuts_reco",&reco_var[38]);
  sel_ncpi0_output_tree->Branch("dllee_failed_boost_reco",&reco_var[39]);
  sel_ncpi0_output_tree->Branch("tot_PE_reco",&reco_var[40]);
  sel_ncpi0_output_tree->Branch("porch_tot_PE_reco",&reco_var[41]);
  sel_ncpi0_output_tree->Branch("dllee_scedr",&reco_var[44]);
  sel_ncpi0_output_tree->Branch("dllee_pot_weight",&reco_var[45]);
  sel_ncpi0_output_tree->Branch("dllee_pi0_flag",&reco_var[48]);
  sel_ncpi0_output_tree->Branch("dllee_nBDTs",&reco_var[50]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_tvweight00",&reco_var[51]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_score00",&reco_var[52]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_tvweight01",&reco_var[53]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_score01",&reco_var[54]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_tvweight02",&reco_var[55]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_score02",&reco_var[56]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_tvweight03",&reco_var[57]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_score03",&reco_var[58]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_tvweight04",&reco_var[59]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_score04",&reco_var[60]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_tvweight05",&reco_var[61]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_score05",&reco_var[62]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_tvweight06",&reco_var[63]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_score06",&reco_var[64]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_tvweight07",&reco_var[65]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_score07",&reco_var[66]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_tvweight08",&reco_var[67]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_score08",&reco_var[68]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_tvweight09",&reco_var[69]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_score09",&reco_var[70]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_score_avg",&reco_var[71]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_score_median",&reco_var[72]);
  sel_ncpi0_output_tree->Branch("dllee_bdt_score_max",&reco_var[73]);
  sel_ncpi0_output_tree->Branch("nu_energy_QE_lepton_reco",&reco_var[76]);
  sel_ncpi0_output_tree->Branch("nu_energy_QE_proton_reco",&reco_var[77]);
  sel_ncpi0_output_tree->Branch("dllee_pi0_weight",&reco_var[78]);
  // ... for CC pi0
  ...
  sel_ccpi0_output_tree->Branch("phiT_reco",&reco_var[29]);
  sel_ccpi0_output_tree->Branch("Q2_reco",&reco_var[30]);
  sel_ccpi0_output_tree->Branch("lepton_length_reco",&reco_var[31]);
  sel_ccpi0_output_tree->Branch("proton_length_reco",&reco_var[32]);
  sel_ccpi0_output_tree->Branch("proton_cos_theta_reco",&reco_var[33]);
  sel_ccpi0_output_tree->Branch("lepton_cos_theta_reco",&reco_var[34]);
  sel_ccpi0_output_tree->Branch("x_true",&reco_var[35]);
  sel_ccpi0_output_tree->Branch("y_true",&reco_var[36]);
  sel_ccpi0_output_tree->Branch("z_true",&reco_var[37]);
  sel_ccpi0_output_tree->Branch("dllee_pass_simple_cuts_reco",&reco_var[38]);
  sel_ccpi0_output_tree->Branch("dllee_failed_boost_reco",&reco_var[39]);
  sel_ccpi0_output_tree->Branch("tot_PE_reco",&reco_var[40]);
  sel_ccpi0_output_tree->Branch("porch_tot_PE_reco",&reco_var[41]);
  sel_ccpi0_output_tree->Branch("dllee_scedr",&reco_var[44]);
  sel_ccpi0_output_tree->Branch("dllee_pot_weight",&reco_var[45]);
  sel_ccpi0_output_tree->Branch("dllee_pi0_flag",&reco_var[48]);
  sel_ccpi0_output_tree->Branch("dllee_nBDTs",&reco_var[50]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_tvweight00",&reco_var[51]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_score00",&reco_var[52]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_tvweight01",&reco_var[53]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_score01",&reco_var[54]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_tvweight02",&reco_var[55]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_score02",&reco_var[56]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_tvweight03",&reco_var[57]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_score03",&reco_var[58]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_tvweight04",&reco_var[59]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_score04",&reco_var[60]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_tvweight05",&reco_var[61]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_score05",&reco_var[62]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_tvweight06",&reco_var[63]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_score06",&reco_var[64]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_tvweight07",&reco_var[65]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_score07",&reco_var[66]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_tvweight08",&reco_var[67]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_score08",&reco_var[68]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_tvweight09",&reco_var[69]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_score09",&reco_var[70]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_score_avg",&reco_var[71]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_score_median",&reco_var[72]);
  sel_ccpi0_output_tree->Branch("dllee_bdt_score_max",&reco_var[73]);
  sel_ccpi0_output_tree->Branch("nu_energy_QE_lepton_reco",&reco_var[76]);
  sel_ccpi0_output_tree->Branch("nu_energy_QE_proton_reco",&reco_var[77]);
  sel_ccpi0_output_tree->Branch("dllee_pi0_weight",&reco_var[78]);
  // ... for dirt nu
  ...
  sel_dirt_output_tree->Branch("phiT_reco",&reco_var[29]);
  sel_dirt_output_tree->Branch("Q2_reco",&reco_var[30]);
  sel_dirt_output_tree->Branch("lepton_length_reco",&reco_var[31]);
  sel_dirt_output_tree->Branch("proton_length_reco",&reco_var[32]);
  sel_dirt_output_tree->Branch("proton_cos_theta_reco",&reco_var[33]);
  sel_dirt_output_tree->Branch("lepton_cos_theta_reco",&reco_var[34]);
  sel_dirt_output_tree->Branch("x_true",&reco_var[35]);
  sel_dirt_output_tree->Branch("y_true",&reco_var[36]);
  sel_dirt_output_tree->Branch("z_true",&reco_var[37]);
  sel_dirt_output_tree->Branch("dllee_pass_simple_cuts_reco",&reco_var[38]);
  sel_dirt_output_tree->Branch("dllee_failed_boost_reco",&reco_var[39]);
  sel_dirt_output_tree->Branch("tot_PE_reco",&reco_var[40]);
  sel_dirt_output_tree->Branch("porch_tot_PE_reco",&reco_var[41]);
  sel_dirt_output_tree->Branch("dllee_scedr",&reco_var[44]);
  sel_dirt_output_tree->Branch("dllee_pot_weight",&reco_var[45]);
  sel_dirt_output_tree->Branch("dllee_pi0_flag",&reco_var[48]);
  sel_dirt_output_tree->Branch("dllee_nBDTs",&reco_var[50]);
  sel_dirt_output_tree->Branch("dllee_bdt_tvweight00",&reco_var[51]);
  sel_dirt_output_tree->Branch("dllee_bdt_score00",&reco_var[52]);
  sel_dirt_output_tree->Branch("dllee_bdt_tvweight01",&reco_var[53]);
  sel_dirt_output_tree->Branch("dllee_bdt_score01",&reco_var[54]);
  sel_dirt_output_tree->Branch("dllee_bdt_tvweight02",&reco_var[55]);
  sel_dirt_output_tree->Branch("dllee_bdt_score02",&reco_var[56]);
  sel_dirt_output_tree->Branch("dllee_bdt_tvweight03",&reco_var[57]);
  sel_dirt_output_tree->Branch("dllee_bdt_score03",&reco_var[58]);
  sel_dirt_output_tree->Branch("dllee_bdt_tvweight04",&reco_var[59]);
  sel_dirt_output_tree->Branch("dllee_bdt_score04",&reco_var[60]);
  sel_dirt_output_tree->Branch("dllee_bdt_tvweight05",&reco_var[61]);
  sel_dirt_output_tree->Branch("dllee_bdt_score05",&reco_var[62]);
  sel_dirt_output_tree->Branch("dllee_bdt_tvweight06",&reco_var[63]);
  sel_dirt_output_tree->Branch("dllee_bdt_score06",&reco_var[64]);
  sel_dirt_output_tree->Branch("dllee_bdt_tvweight07",&reco_var[65]);
  sel_dirt_output_tree->Branch("dllee_bdt_score07",&reco_var[66]);
  sel_dirt_output_tree->Branch("dllee_bdt_tvweight08",&reco_var[67]);
  sel_dirt_output_tree->Branch("dllee_bdt_score08",&reco_var[68]);
  sel_dirt_output_tree->Branch("dllee_bdt_tvweight09",&reco_var[69]);
  sel_dirt_output_tree->Branch("dllee_bdt_score09",&reco_var[70]);
  sel_dirt_output_tree->Branch("dllee_bdt_score_avg",&reco_var[71]);
  sel_dirt_output_tree->Branch("dllee_bdt_score_median",&reco_var[72]);
  sel_dirt_output_tree->Branch("dllee_bdt_score_max",&reco_var[73]);
  sel_dirt_output_tree->Branch("nu_energy_QE_lepton_reco",&reco_var[76]);
  sel_dirt_output_tree->Branch("nu_energy_QE_proton_reco",&reco_var[77]);
  sel_dirt_output_tree->Branch("dllee_pi0_weight",&reco_var[78]);
  // ... for extbnb
  ...
  sel_extbnb_output_tree->Branch("phiT_reco",&reco_var[29]);
  sel_extbnb_output_tree->Branch("Q2_reco",&reco_var[30]);
  sel_extbnb_output_tree->Branch("lepton_length_reco",&reco_var[31]);
  sel_extbnb_output_tree->Branch("proton_length_reco",&reco_var[32]);
  sel_extbnb_output_tree->Branch("proton_cos_theta_reco",&reco_var[33]);
  sel_extbnb_output_tree->Branch("lepton_cos_theta_reco",&reco_var[34]);
  sel_extbnb_output_tree->Branch("x_true",&reco_var[35]);
  sel_extbnb_output_tree->Branch("y_true",&reco_var[36]);
  sel_extbnb_output_tree->Branch("z_true",&reco_var[37]);
  sel_extbnb_output_tree->Branch("dllee_pass_simple_cuts_reco",&reco_var[38]);
  sel_extbnb_output_tree->Branch("dllee_failed_boost_reco",&reco_var[39]);
  sel_extbnb_output_tree->Branch("tot_PE_reco",&reco_var[40]);
  sel_extbnb_output_tree->Branch("porch_tot_PE_reco",&reco_var[41]);
  sel_extbnb_output_tree->Branch("dllee_scedr",&reco_var[44]);
  sel_extbnb_output_tree->Branch("dllee_pot_weight",&reco_var[45]);
  sel_extbnb_output_tree->Branch("dllee_pi0_flag",&reco_var[48]);
  sel_extbnb_output_tree->Branch("dllee_nBDTs",&reco_var[50]);
  sel_extbnb_output_tree->Branch("dllee_bdt_tvweight00",&reco_var[51]);
  sel_extbnb_output_tree->Branch("dllee_bdt_score00",&reco_var[52]);
  sel_extbnb_output_tree->Branch("dllee_bdt_tvweight01",&reco_var[53]);
  sel_extbnb_output_tree->Branch("dllee_bdt_score01",&reco_var[54]);
  sel_extbnb_output_tree->Branch("dllee_bdt_tvweight02",&reco_var[55]);
  sel_extbnb_output_tree->Branch("dllee_bdt_score02",&reco_var[56]);
  sel_extbnb_output_tree->Branch("dllee_bdt_tvweight03",&reco_var[57]);
  sel_extbnb_output_tree->Branch("dllee_bdt_score03",&reco_var[58]);
  sel_extbnb_output_tree->Branch("dllee_bdt_tvweight04",&reco_var[59]);
  sel_extbnb_output_tree->Branch("dllee_bdt_score04",&reco_var[60]);
  sel_extbnb_output_tree->Branch("dllee_bdt_tvweight05",&reco_var[61]);
  sel_extbnb_output_tree->Branch("dllee_bdt_score05",&reco_var[62]);
  sel_extbnb_output_tree->Branch("dllee_bdt_tvweight06",&reco_var[63]);
  sel_extbnb_output_tree->Branch("dllee_bdt_score06",&reco_var[64]);
  sel_extbnb_output_tree->Branch("dllee_bdt_tvweight07",&reco_var[65]);
  sel_extbnb_output_tree->Branch("dllee_bdt_score07",&reco_var[66]);
  sel_extbnb_output_tree->Branch("dllee_bdt_tvweight08",&reco_var[67]);
  sel_extbnb_output_tree->Branch("dllee_bdt_score08",&reco_var[68]);
  sel_extbnb_output_tree->Branch("dllee_bdt_tvweight09",&reco_var[69]);
  sel_extbnb_output_tree->Branch("dllee_bdt_score09",&reco_var[70]);
  sel_extbnb_output_tree->Branch("dllee_bdt_score_avg",&reco_var[71]);
  sel_extbnb_output_tree->Branch("dllee_bdt_score_median",&reco_var[72]);
  sel_extbnb_output_tree->Branch("dllee_bdt_score_max",&reco_var[73]);
  sel_extbnb_output_tree->Branch("nu_energy_QE_lepton_reco",&reco_var[76]);
  sel_extbnb_output_tree->Branch("nu_energy_QE_proton_reco",&reco_var[77]);
  sel_extbnb_output_tree->Branch("dllee_pi0_weight",&reco_var[78]);
  // ... for bnb data
  ...
  */
  ...
  for ( int i=0; i < bnb_evweight_tree->GetEntries(); i++ ) {
    ...
    if ( sel_bnb_event_map.find(rse) != sel_bnb_event_map.end() ) {
      ...
      if ( std::abs(nu_energy_true - reco_var[42]) > 0.01 ) continue;
