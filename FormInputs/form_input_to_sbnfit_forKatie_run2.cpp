...
int main() {
  ...
  // ... for event lists
  const char* sel_bnb_eventlist_file      = "/uboone/app/users/kmason/SystematicInfo/2021Sep8_fullpi0selection_bnb_overlay_run2.txt";
  const char* sel_bnb_lowE_eventlist_file = "/uboone/app/users/kmason/SystematicInfo/empty.txt";
  const char* sel_nue_eventlist_file      = "/uboone/app/users/kmason/SystematicInfo/2021Sep8_fullpi0selection_nue_intrinsic_run2.txt";
  ...
  const char* sel_extbnb_eventlist_file   = "/uboone/app/users/kmason/SystematicInfo/2021Sep8_fullpi0selection_ext_run3.txt";
  // ... for data event list
  ...
  // ... for output
  const char* output_file = "/uboone/app/users/kmason/SystematicInfo/input_to_sbnfit_v55_Jun20_withExtraGENIE_pi0_run2_Sep8.root";

  // Initalize the number of reco variables
  const int N_var = 7;  // 7 reco variables + the pi0 weight
  
  //** Read in lists of selected events and their reconstructed energies **//
  
  // Initialize maps to hold selected events
  std::map<std::pair<std::array<int,3>,double>,std::array<double,N_var>> sel_bnb_event_map;
  ...
  std::array<int,3> rse;
  std::pair<std::array<int,3>,double> rsee;  // RSE + true neutrino energy
  double reco_energy;
  ...
  if ( sel_bnb_eventlist.is_open() ) {
    while ( getline(sel_bnb_eventlist, line) ) {
      ...
      rsee.first = rse;
      rsee.second = std::stof(split_line[N_var+3]);  // true neutrino energy
      for ( int i=0; i<N_var; i++ ) reco_var[i] = std::stof(split_line[i+3]);
      sel_bnb_event_map.emplace(rsee,reco_var);
    }
  ...
  for ( int i=0; i < bnb_evweight_tree->GetEntries(); i++ ) {
    ...
    rsee.first[1] = subrun;
    rsee.first[2] = event;
    rsee.second = nu_energy_true;
    
    // If there's a corresponding entry in the selected map...
    // Loop over the selected map...
    for ( auto sel_bnb_event_map_iter=sel_bnb_event_map.begin(); sel_bnb_event_map_iter!=sel_bnb_event_map.end(); sel_bnb_event_map_iter++ ) {
      // If this is the corresponding entry based on RSE + true neutrino energy...
      if ( sel_bnb_event_map_iter->first.first == rsee.first and std::abs(sel_bnb_event_map_iter->first.second - rsee.second) < 0.01 ) {
	// Get the reconstructed energy
	reco_var = sel_bnb_event_map_iter->second;
