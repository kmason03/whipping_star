						...
						for(int sin22thi_second = 0; sin22thi_second <sin22th_grdpts; sin22thi_second++){
									...
									for(int j = 0; j < nBins; j++){
											...
											if(i==j){
												...
												if (truespec_second.collapsed_vector[i] >0 && fakeData[i] >0){
													cov[i][j] += 3.0 / (1.0/fakeData[i] + 2.0/truespec_first.collapsed_vector[i]);
													// poisson version
							...
							for(int i = 0; i < nBins; i++){
								for(int j = 0; j < nBins; j++){
									...
									chisqTest += (fakeData[i] - truespec_second.collapsed_vector[i])*invcov[i][j]*(fakeData[j] - truespec_second.collapsed_vector[j]);
								}
							}
