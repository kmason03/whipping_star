...
// Changelog for q2_1m1p:
// thigns to change:
// xml path
// nbins
// makespectraplotscomp function
// continue after makespec function
// getprecomp spectra bool islogname
// adjust makespectraplotscomp function
// std::string xml = "/cluster/tufts/wongjiradlab/jmills09/whipping_star/xml/numudisappearance_q2.xml";
...
int main(int argc, char* argv[]){
	...
	if(!gen){
		...
		if (statsOnly){
			if (!shapeonly){
				chiFileName = "rterms_statsonly_1x_nonfreq.txt";
			}
			else{
				chiFileName = "rterms_statsonly_shapeonly_1x_nonfreq.txt";
			}
			if (doOneMass){
				if (!shapeonly){
					chiFileName = "rterms_statsonly_1x_nonfreq_pt_"+ZeroPadNumber(miToDo)+".txt";
				}
				else{
					chiFileName = "rterms_statsonly_shapeonly_1x_nonfreq_pt_"+ZeroPadNumber(miToDo)+".txt";
				}
			}
		}
		else{
			if (!shapeonly){
				chiFileName = "rterms_1x_nonfreq.txt";
			}
			else{
				chiFileName = "rterms_shapeonly_1x_nonfreq.txt";
			}
			if (doOneMass){
				if (!shapeonly){
					chiFileName = "rterms_1x_nonfreq_pt_"+ZeroPadNumber(miToDo)+".txt";
				}
				else{
					chiFileName = "rterms_shapeonly_1x_nonfreq_pt_"+ZeroPadNumber(miToDo)+".txt";
				}
			}
		}
		...
		getPrecompSpec(a_sinsqSpec, dm2_grdpts, dm2_hibound, dm2_lowbound, xml, scaleFactor, true); //Change to false for q2 (or newly generated mass specs)
		...
		for(int mi_base = miBaseStart; mi_base <miBaseLimit; mi_base++){
			for(int sin22thi_base = 0; sin22thi_base <sin22th_grdpts; sin22thi_base++){
				...
				makeSpectraPlotsComp_Ratio(cvSpec.collapsed_vector, truespec_first.collapsed_vector, sin22thi_base, mi_base, sin22th_first, pow(mnu_base,2), nBins, "exp_spectras/");
				// makeSpectraPlotsComp_Ratio(cvSpec.collapsed_vector, truespec_first.collapsed_vector, sin22thi_base, mi_base, sin22th_first, pow(mnu_base,2), nBins, "exp_spec_q2");
				// assert (1==2);
				continue;
				...
				if(shapeonly){
					scaleCovShapeOnly(cov, covFracSys_base, thisGridCVSpec.collapsed_vector, truespec_first.collapsed_vector, nBins); // cov, covfraccollapsed, obs, exp(gets modified), nbins
					// makeSpectraPlotsComp(cvSpec.collapsed_vector, truespec_first.collapsed_vector, sin22thi_base, mi_base, sin22th_first, pow(mnu_base,2));
				}
				// Shape+Rate
				else{
					scaleCovShapeRate(cov, covFracSys_base, thisGridCVSpec.collapsed_vector, truespec_first.collapsed_vector, nBins); // cov, covfraccollapsed, obs, exp, nbins
				}
				...
				calcRTerm(detTermTest, chisqTest, thisGridCVSpec.collapsed_vector, truespec_first.collapsed_vector, invcov, cov, nBins); // obs, exp, inv, cov, nbins
